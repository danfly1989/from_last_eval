...
/*Due to the logic of process redirection tokens,
the last_in_fd will only ever be -1 in case of no
< redirect input*/
...
/*Output redirect flag works the same way as above but
needed to be a separate variable to ensure that it is
the result of > detection only*/
...
are all in hande_redir.c. This leads to where the opening happens and
related functions are now in redir.c. This also ensures the last_fd
of each type is only switched "on" by its associated symbol*/
...
we. Negation means failures activate instant return. If one redirect
